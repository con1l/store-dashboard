...
# Get original full HTML from git
...
funcs_before = re.findall(r'function\s+(\w+)', content)
print(f'Functions before: {funcs_before}')
...
# 2. Replace ONLY handleUpload - find exact boundaries
...
funcs_after = re.findall(r'function\s+(\w+)', content)
print(f'After size: {len(content)}')
print(f'Functions after: {funcs_after}')
print(f'Has init(): {("function init()" in content)}')
print(f'Has renderTopCards: {("renderTopCards" in funcs_after)}')
