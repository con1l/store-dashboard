...
# Change 2: Replace handleUpload using end marker
start = content.find('async function handleUpload(file) {')
end_marker = '\n}\n\n\n(function init() {'
end_pos = content.find(end_marker, start)
func_end = content.rfind('\n}', start, end_pos) + 2  # include \n}

print(f'Replacing handleUpload: {start} to {func_end} ({func_end-start} chars)')
