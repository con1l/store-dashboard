...
# 2. Replace handleUpload body (brace-matched)
...
# 3. Fix initApp: replace init() call with the body of init IIFE inlined
# First, extract the IIFE body
iife_start = content.find('(function init() {')
iife_bs = content.find('{', iife_start)
depth = 0; ipos = iife_bs
while ipos < len(content):
    if content[ipos] == '{': depth += 1
    elif content[ipos] == '}':
        depth -= 1
        if depth == 0: break
    ipos += 1
iife_body = content[iife_bs+1:ipos]

# Now replace initApp's init() call with the IIFE body inlined
content = content.replace(
    '  init();\n}',
    '  // init logic (inlined from IIFE)\n' + iife_body + '\n}'
)

# Remove the IIFE since its body is now inlined into initApp
# Find (function init() ... })();
iife_full_start = content.find('(function init() {')
depth = 0; fpos = content.find('(', iife_full_start)
while fpos < len(content):
    if content[fpos] == '(': depth += 1
    elif content[fpos] == ')': depth -= 1
    if depth == 0 and fpos > iife_full_start:
        # include }); after )
        end_iife = content.index('});', fpos) + 3 if content[fpos:fpos+3] == ');' else fpos + 1
        break
    fpos += 1
...
for i, s in enumerate(scripts):
    if len(s) > 20:
        with open(f'_f{i}.js', 'w', encoding='utf-8') as out:
            out.write(s.strip())
